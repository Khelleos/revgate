...
import { parseArgs, type CliOptions, type ParsedArgs } from "../../src/cli/args.js";
...
test("parseArgs: no args is bad usage, not a hook", () => {
  // There is no hook behind bare `revgate` any more, so a stale hooks.json that
  // still invokes it must get a loud exit-2 explanation, not a review UI.
...
test("parseArgs: an empty separate-token value is an error too", () => {
  // The inline form already fails; a separate empty token must too, or the
  // `if (v)` at the call site drops it in silence: `-o ""` leaves the
  // annotations on stdout and `-I ""` reviews the whole tree.
...
test("parseArgs: a boolean switch rejects an inline value instead of inverting it", () => {
  // `--no-history=false` reads as "keep history", so accepting it as bare
  // `--no-history` inverts the caller's intent in silence. The caller is an LLM
  // with no way to notice, so an explicit usage error sends it to exit 2.
  for (const flag of [
    "--no-history",
    "--staged",
    "--exit-code-on-comments",
