...
import { BUILTIN_THEMES, PALETTE_KEYS } from "../../src/store/palettes.js";
import type { ThemeListing } from "../../src/store/theme-config.js";
import { createRepo } from "../helpers/repo.js";
import type { DiffFile, ReviewSubmission } from "../../src/shared/types.js";
...
test("startReviewServer: the split-out stylesheet and script are served typed", async (t) => {
  // The markup only references these; a wrong MIME type is a page that parses
  // the stylesheet as text and never runs the script, with a 200 on both.
  const server = await serve(t);
  for (const [file, type] of [
    ...
    const res = await get(`${server.url}${file}`);
    assert.equal(res.status, 200, file);
    assert.equal(res.headers.get("content-type"), type, file);
    assert.ok((await res.text()).length > 0, `${file} is empty`);
  }
});

test("PUBLIC_DIR: the same resolution finds public/ from dist/ as from src/", async (t) => {
  // `PUBLIC_DIR` is `__dirname`-relative, and this module sits one level below
  // `src/` — so the compiled copy sits one level below `dist/`. Getting the
  // number of `..` steps wrong costs nothing under tsx (the test above still
  // passes) and serves a 404 for every asset in an installed build, which is a
  // blank review page and a gate the reviewer cannot resolve. `src/server` and
  // `dist/server` sit at the same depth, so this one equality covers both.
  const server = await serve(t);
  assert.equal((await get(server.url)).status, 200, "public/ is not where PUBLIC_DIR says");
  assert.equal(PUBLIC_DIR, path.join(repoRoot, "public"));
});
...
test("public/index.html: the markup pulls in the split-out stylesheet and script", async () => {
  const html = await readFile(path.join(repoRoot, "public", "index.html"), "utf8");

  assert.match(html, /<link rel="stylesheet" href="\/app\.css"/, "app.css is not linked");
  assert.match(html, /<script src="\/app\.js" defer><\/script>/, "app.js is not loaded with defer");

  // The split is only safe while the page carries no second copy of either.
  assert.doesNotMatch(html, /<style>/, "a style block is left in the markup");
  assert.doesNotMatch(html, /<script>/, "an inline script is left in the markup");
});

test("public/app.js: the picker is in the script and no localStorage theme code survives", async () => {
  ...
  const script = await readFile(path.join(repoRoot, "public", "app.js"), "utf8");

  assert.match(script, /id: "theme-slot"/, "the header has no slot for the picker");
  assert.match(script, /id: "theme-picker"/, "no picker is built");
  assert.match(script, /"\/api\/themes"/, "the page never asks for the themes");
  assert.match(script, /"\/api\/theme"/, "a pick is never persisted");
  ...
  assert.doesNotMatch(script, /localStorage/, "localStorage theme code is left in the page");
  assert.doesNotMatch(script, /data-theme/, "the old data-theme switch is left in the page");
  ...
  for (const id of ["dark-modern", "light-modern"]) {
    assert.match(script, new RegExp(`"${id}"`), `the page never resolves system to ${id}`);
    assert.ok(
      BUILTIN_THEMES.some((theme) => theme.id === id),
      `public/app.js hard-codes ${id}, which is no longer a built-in`,
    );
  }
});
...
test("POST /api/submit: the body reaches the normalizer, not the caller, raw", async (t) => {
  // The route's own share of this: `test/server/normalize.test.ts` asserts what
  // normalizeSubmission produces, so one case here is enough to prove the route
  // calls it at all rather than resolving the gate with whatever was posted.
  const server = await startReviewServer(context());
  t.after(() => server.close());
...
test("POST /api/submit: the file set the normalizer filters against is this review's", async (t) => {
  // The route supplies `known`; normalize.test.ts covers what happens to a path
  // outside it. If the route ever passed the wrong set, a comment on a file
  // nobody reviewed would reach the agent as a directive.
  const server = await startReviewServer(context());
  t.after(() => server.close());
