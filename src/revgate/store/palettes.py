"""The built-in colour themes.

Every palette is a complete map over `PALETTE_KEYS`; nothing merges over a
base. The page CSS hard-copies two of them, and `public/app.css` is the one
place that has to be re-checked by eye when a built-in is retuned.
"""
...
def is_known_theme_id(theme_id: object) -> bool:
    """True for the five built-ins and for `system`.

    Takes `object`, because the caller is validating a value that came
    straight off `json.loads` and has proved nothing about it yet.
    """
    if not isinstance(theme_id, str):
        return False
    return theme_id == SYSTEM_THEME_ID or any(t.id == theme_id for t in BUILTIN_THEMES)
